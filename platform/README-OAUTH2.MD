...
Second, we need to setup environment for the examples app. Copy `.env.example` to `.env` and set the following
...
Here we have OAUTH2_CLIENT_SECRET_PLAIN in plain because it will be used for api request when exchanging authorization code for tokens. NEXT_PUBLIC_OAUTH2_MODE tells that the examples app will run not with platform OAuth client but with OAuth 2.0 client, so in `src/pages/_app.tsx` we will use CalOAuthProvider and not CalProvider.

## Usage
1. Before running the example app, run `rm -f prisma/dev.db && yarn prisma db push` to reset its SQLite. If you don't and there are users already then an entry for the `admin@example.com` will not be created.
